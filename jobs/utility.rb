...
SCHEDULER.every '1m', :first_in => 0 do |job|
  ...
  sql = "SELECT DISTINCT(utility_name) FROM utility_history"
  utility_rows = db.query(sql)
  utility_items = utility_rows.map do |row|
    utility_name = row['utility_name']
    unless utility_name.nil?
      sql = "
      SELECT unit_name, sum(units_delta) as day_total_units
      FROM utility_history 
      WHERE utility_name='" + utility_name + "' AND unit_name IS NOT null AND updated_on >= CURDATE()
      GROUP BY unit_name
      "
